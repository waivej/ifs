...
Public Sub GetBitmapPixels(ByVal pic As PictureBox, ByRef pixels() As RGBTriplet, ByRef bits_per_pixel As Integer, Optional Clear = False)
            ...
            For Y = 0 To hgt - 1
                For X = 0 To wid - 1
                    With pixels(X, Y)
                        ...
                        .rgbRed = (.rgbRed * 255) / 31
                        .rgbGreen = (.rgbGreen * 255) / 63
                        .rgbBlue = (.rgbBlue * 255) / 31
                    End With
                Next X
            Next Y
